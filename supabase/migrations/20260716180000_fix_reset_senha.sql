-- Fix resetar_senha_tecnico to search in both perfis and tecnicos for the target empresa_id
...
BEGIN
    ...
    -- Descobre o empresa_id do técnico na tabela perfis
    SELECT empresa_id INTO v_target_empresa_id
    FROM public.perfis
    WHERE id = p_tecnico_id;

    -- Caso o técnico não esteja em perfis, busca em tecnicos
    ...
    IF v_target_empresa_id IS NULL THEN
        RAISE EXCEPTION 'Técnico não encontrado na tabela de perfis nem em tecnicos.';
    END IF;
